function [R11,R22,r,R1,R2,R3]=correlation(u,v,w,Lx)
    scaling = 1;
    ...
    R1=ifftn(Rij_x,NFFT)/std2(u)^2./prod(NFFT);
    % y-component
    NFFT = 2.^nextpow2(size(v_fft));
    R2=ifftn(Rij_y,NFFT)/std2(v)^2./prod(NFFT);
    % z-component
    NFFT = 2.^nextpow2(size(w_fft));
    R3=ifftn(Rij_z,NFFT)/std2(w)^2./prod(NFFT);
